#https://doi.org/10.1016/j.jeconom.2022.12.008
#https://sites.google.com/view/changbozhu/home
...
cv.hs <- function(Z, model = 2, maxp = min(10, floor(ncol(Z)/2))){
  d <- dim(Z)[1]
  n <- dim(Z)[2]
  ...
  ERR <- sapply(c(1:n.test), function(t){
    ...
    err <- sapply(c(1:maxp), function(p){
...
getAlpha <- function(Z, mu, model, p){
  ...
  # what is U? It's kinda R(t). R(t) - theta * Q. But it doesn't return the whole Q, it 
  # returns the vector u2 for some reason
  U <- getU(Z, mu, model)
  
  d <- dim(Z)[1]
  n <- dim(U)[2] # why fewer dims than Z sometimes? Because when model = 2 you're differencing
  
  # what is V?
  # I think model == 1 is SAR, where Rt = x_t (-) \mu_x
  # I think model == 2 is DSAR, where Rt = x_t+1 (-) x_t
  # 
  ...
  # What is D? 
  # D must contain the <Rt - \hat{\mu}, R_t+k - \hat{\mu}>
  ...
  # This is the vector [\hat{\lambda}_1, \hat{\lambda}_2, ..., \hat{\lambda}_p]
  # This bit implements (7), so D must contain the <Rt - \hat{\mu}, R_t+k - \hat{\mu}>
  ...
  # this is the matrix of autocovariances
  ...
  par <- list("Z" = Z, "U" = U, "alpha" = alpha, "model" = model, "mu" = mu)
  return(par)
}

getU <- function(Z, mu=NULL, model){
  # See page 5. This is columnwise storing theta * u2 for each observation. 
  # So technically NOT all of R(t), but so what...
  n <- dim(Z)[2]
  
  if(model==1){
    # model == 1 is SAR, where Rt = x_t (-) \mu_x
    # x_t (-) \mu_x = theta * Q_{\mu_x, x_t}
    # Q_{\mu_x, x_t} = u1 (x) u2 - u2 (x) u1
    # u1 = \mu_x
    # u2 = (x_t - <x_t, u1>u1) / norm(...)
...
# I don't know why this is called "apply S." I think what it does is apply
# R
...
predict.hs <- function(Z, U, mu, alpha, model, projection, PredictorIndex){
  # Z is the data 
  # U is sorta-kinda the R(t)
  # mu is the frechet mean of the dataset 
  # alpha are estimated params 
  # model switches SAR or DSAR 
  # projection does nothing in this code, but I think it has to do with the 
  #            stuff in the very last paragraph of page 7
  # PredictorIndex tells how many lags to use to compute \hat{R}
  #.               and it makes a lot of assumptions. Must be length p, the end
  #                
  p <- length(alpha)
  ...
  V[,1] <- v # because R ^ 0 = Identity operator
  for(i in c(2:N)){
    # I think this is just recomputing the same \hat{R} every time and applying it 
    # to a new v
    V[,i] <- applyS(V[,(i-1)], Z, U, mu, alpha, model, PredictorIndex)
  }
  
  # top of page 4. exp(L) = sum from i = 0 ^ \infty of L^i / i!
  # And then page 7, the prediction is x_{t+1} = exp(R_{t+1}) * \mu (in the SAR case)
  # So we calculate the first N = 10 terms of the infinite sum and approximate
  # So V must column-wise store R^i %*% \mu (in the SAR case)
