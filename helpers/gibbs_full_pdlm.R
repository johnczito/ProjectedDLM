gibbs_full_pdlm <- function(U, FF, priorparams, Gam0, gam0, G0, W0, r0, ndraw, burn, thin){
  ...
  for (m in 1:M) {
    ...
    #---------------------------------------------------------------------------
    #  draw from p(Gam | ...) ONLY WORKS FOR n = 2
    #---------------------------------------------------------------------------
    
    #e = Y[, 1] - Mu[, 1] - gam * (Y[, 2] - Mu[, 2])
    #Gam = as.matrix( draw_invgamma_posterior(1, e, a0, b0) )
    #Gam = as.matrix(sample_conjugate_posterior_invgamma(1, e, c(a0, b0)))
    ...
    M = 1 / ((1 / M0) + (1 / Gam) * sum( (Y[, 2] - Mu[, 2]) ^ 2 ))
    m1 = M * ((1 / Gam) * sum( (Y[, 1] - Mu[, 1]) * (Y[, 2] - Mu[, 2]) ) + m0 / M0)
    
    gam = rnorm(1, m1, sqrt(M))
